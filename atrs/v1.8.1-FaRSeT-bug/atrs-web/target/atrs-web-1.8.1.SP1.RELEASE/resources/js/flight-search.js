...
(function() {
    ...
    $(function init() {
        ...
        $(document).on('submit', '#flights-search-form',
                handlers.onSubmitFlightsSearchForm);
        $(document).on('submit', '#flights-select-form',
                handlers.onSubmitFlightsSelectForm);
        $(document).on('submit', '#login-form', handlers.onSubmitLoginForm);
        $(document).on('click', '#reserve-without-login-button',
                handlers.onClickReserveWithoutLoginButton);
        ...
        // - 初期空席照会
        var isInitialSearchUnnecessary = $(
                'meta[name="isInitialSearchUnnecessary"]').attr('content');
    ...
    var handlers = {
        ...
        onSubmitFlightsSelectForm : function(e) {
            ...
            $.ajax({
                ...
                default:
                    ...
                    // エラーメッセージ表示
                    $('#flights-select-form-messages').empty().toggleClass('hidden', false).append($('<li>').text(errorText));
                }
            });
        },
        ...
        onShowLoginModal : function() {
            $(this).find('input[name=membershipNumber], input[name=password]')
                    .val('');
        },
        ...
        onSubmitLoginForm : function(e) {
            ...
            $.ajax({
                ...
                // エラーメッセージ表示
                $('#login-messages').empty().append($('<li>').text(errorText)).toggleClass('hidden', false);
            });
        },
    ...
    function formatFlightSearchParam(homeward) {
        var flightsSearchParam = _.reduce($('#flights-search-form')
                .serializeArray(), function(acc, item) {
    ...
    function reserveFlights(force) {
        ...
        // 選択フライト
        var flightPropertyNames = [ 'flightName', 'fareTypeCd', 'depDate',
                'boardingClassCd' ];
        $('#outward-flights-table:visible input[name=outward-flight-select]:checked, #homeward-flights-table:visible input[name=homeward-flight-select]:checked').each(function(idx, el) {
            _.forEach(flightPropertyNames, function(propertyName) {
                ...
                $form.append($('<input>').attr('name', inputName).attr('type', 'hidden').val($(el).data(propertyName)));
            });
        });
        ...
        for ( var i in owCriteria) {
            $form.append($('<input>').attr('type', 'hidden').attr('name', i)
                    .val(owCriteria[i]));
        }
        $form.append($('<input>').attr('type', 'hidden').attr('name',
                'outwardDate').val(owCriteria.depDate));
        var hwCriteria = homewardFlightsTable.getSearchCriteria();
        $form.append($('<input>').attr('type', 'hidden').attr('name',
                'homewardDate').val(hwCriteria.depDate));
        ...
        if (force)
            $(document).off('submit', '#flights-select-form',
                    handlers.onSubmitFlightsSelectForm);
    ...
    function FlightsTable(selector) {
        ...
        var self = this, direction = selector.match(/outward/) ? 'outward'
                : selector.match(/homeward/) ? 'homeward' : '', $this = $(selector), $table = $(selector
                + '-table'), $pager = $(selector + '-pager'), $fightTableMessages = $(selector
                + '-messages'), $dateLabel = $this.find('.pager-current-date'), $prevDateButton = $this
                .find('.prev-date-button'), $nextDateButton = $this
                .find('.next-date-button');
        ...
        var handlers = {
            ...
            onClickDateChangingButton : function(e) {
                ...
                currentParam.depDate = moment(currentParam.depDate, DATE_FORMAT)
                        .add(offset, 'd').format(DATE_FORMAT);
            ...
            onChangeFlightSelect : function() {
                var $this = $(this);
                $this.parents('tbody').find('td')
                        .toggleClass('selected', false);
                $this.parents('td').toggleClass('selected', true);

                // 選択フライトの data 属性から hidden 要素を生成
                var index = (direction === 'outward' ? 0 : 1);
                var flightPropertyNames = [ 'flightName', 'fareTypeCd',
                        'depDate', 'boardingClassCd' ];
                _.forEach(flightPropertyNames, function(propertyName) {
                    var inputName = 'selectFlightFormList[' + index + '].'
                            + propertyName;
                    var $input = $('#flights-select-form input[name="'
                            + inputName + '"]');
        ...
        this.update = function(param) {
            ...
            // エラーメッセージ初期化、非表示化
            $('#flights-select-form-messages').toggleClass('hidden', true)
            ...
            var startDate = moment($('#outwardDate').data('date-start-date'),
                    DATE_FORMAT);
            var endDate = moment($('#outwardDate').data('date-end-date'),
                    DATE_FORMAT);
            $prevDateButton.parent().toggleClass('disabled',
                    (+depDate <= +startDate));
            $nextDateButton.parent().toggleClass('disabled',
                    (+depDate >= +endDate));
            ...
            $.ajax({
                ...
                timeout : 15000
                    ...
                    function(data) {
                        ...
                        var $form = $('#flights-select-form');
                        var index = (direction === 'outward' ? 0 : 1);
                        var flightName = $form.find('input[name="selectFlightFormList[' + index + '].flightName"]').val();
                        var fareTypeCd = $form.find('input[name="selectFlightFormList[' + index + '].fareTypeCd"]').val();
                        var depDate = $form.find('input[name="selectFlightFormList[' + index + '].depDate"]').val();
                        var boardingClassCd = $form.find('input[name="selectFlightFormList[' + index + '].boardingClassCd"]').val();

                        var flightSelector = '[data-flight-name="' + flightName + '"]' + '[data-fare-type-cd="' + fareTypeCd + '"]' + '[data-dep-date="' + depDate + '"]' + '[data-boarding-class-cd="'
                                + boardingClassCd + '"]:enabled';
                ...
                if (400 <= xhr.status && xhr.status <= 499 && contentType !== null && contentType.indexOf('json') !== -1) {
    ...
    function validateSelectedTime() {
        ...
        var reserveIntervalTime = $('meta[name="reserveIntervalTime"]').attr(
                'content');
        var outwardArrTime = moment(
                $outwardFlight.data('depDate') + ' '
                        + $outwardFlight.data('arrTime'), DATE_TIME_FORMAT)
                .add(reserveIntervalTime, 'm');
        var homewardDepTime = moment($homewardFlight.data('depDate') + ' '
                + $homewardFlight.data('depTime'), DATE_TIME_FORMAT);

        return homewardDepTime.isSame(outwardArrTime)
                || homewardDepTime.isAfter(outwardArrTime);
    }
    ...
    function setValidator() {
        ...
        $('#flights-select-form').parsley({
            ...
            if (!validateSelectedTime()) {
                $('#flights-select-form-messages').toggleClass('hidden', false).append($('<li>').text(MSG_INVALID_FLIGHT_TIME));
            ...
            $flightsSelectFormMessages.toggleClass('hidden', false);
        });
	 ...
	 *
	 * @bug_id        3 (新)
	 * @bug_title     状態管理の不備(復路の日付がリセットされる)
	 * @author        Sota Toyama
	 * @see           「FaRSeTトレーニング用のATRSのサンプル不具合の案」
	 *
	 * @purpose
	 * - 往路日付変更時に、復路の日付を意図的に上書きする処理を追加する。
	 * - ある操作が、関連する他の状態を意図せず破壊してしまうという
	 * 状態管理の不具合を再現する。
	 *
	 * @modification
	 * - 往路日付ピッカー(#outwardDate)の 'changeDate' イベントを監視し、
	 * 復路日付ピッカー(#homewardDate)を同じ日付で更新する処理を追記。
	 *
	 * @impact
	 * - 空席照会画面(B101)の往復検索の操作にのみ影響。
	 *
	 * @revert
	 * - このブロック全体を削除してください。
	 *
	...
	// --- BUGGY CODE ---
	$(function() {
	  // 往路日付(#outwardDate)の変更を監視
	  $('#outwardDate').on('changeDate', function(e) {
	    // 復路日付(#homewardDate)を、往路の新しい日付で上書きしてしまう
	    $('#homewardDate').datepicker('update', e.date);
	  });
	});
