...
public class MemberRegisterForm implements IMemberForm, Serializable {
     ...
     *
     * @purpose
     * - 外部設計書C101で定義された「氏名(姓)」の必須項目チェックを
     * 意図的に無効化する。
     * - 最も基本的なバリデーションである必須チェックの漏れという不具合を再現する。
     *
     * @modification
     * - 「氏名(姓)」に対応するプロパティ(kanjiFamilyName)に付与されている
     * @NotNull アノテーションをコメントアウト。
     *
     * @impact
     * - 会員登録画面(C101)の「氏名(姓)」の入力チェックにのみ影響。
     * - 会員情報変更画面(C201)は別フォームのため影響なし。
     *
     * @revert
    ...
    // --- BUGGY CODE ---
    // 必須チェックを無効化するため、@NotNullをコメントアウト
    // @NotNull
