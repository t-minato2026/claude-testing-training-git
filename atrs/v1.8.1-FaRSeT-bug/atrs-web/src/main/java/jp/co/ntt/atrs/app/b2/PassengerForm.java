...
//FaRSeT(Bug#15): 不具合実装に伴い、このimport文は未使用となるためコメントアウト
...
public class PassengerForm implements Serializable {
     ...
     *
     * @bug_id        15
     * @bug_title     入力仕様違反(全角数字入力)
     * @author        Sota Toyama
     * @see           「FaRSeTトレーニング用のATRSのサンプル不具合の案」
     *
     * @purpose
     * - ユーザーの多様な入力パターンを想定し、システムの入り口(UI)で不正なデータを食い止める「入力バリデーション」の重要性を学習させます。
     *
     * @modification
     * - 搭乗者情報フォーム(PassengerForm.java)の会員番号(membershipNumber)フィールドに付与されていた、半角数字をチェックする`@HalfWidthNumber`アノテーションをコメントアウトしました。
     *
     * @impact
     * - これによりサーバーサイドでの半角数字チェックが無効化され、全角数字が入力された場合に後続の処理で予期せぬエラーが発生するようになります。
     *
     * @revert
     * - このFaRSeTコメントブロックを削除し、直下の`@HalfWidthNumber`のコメントアウトを解除してください。
     *
     ****************************************************************/
    // @HalfWidthNumber
