...
<%@ taglib prefix="t"
    uri="http://terasoluna.org/tags"%>
<%@ taglib prefix="f"
    uri="http://terasoluna.org/functions"%>
<%@ taglib prefix="c"
 ...
 *
 * @bug_id        6
 * @bug_title     パスワードの平文表示
 * @author        Sota Toyama
 * @see           「FaRSeTトレーニング用のATRSのサンプル不具合の案」
 *
 * @purpose
 * - UI上の基本的なセキュリティ対策（ショルダーハッキング対策）の重要性を学習させます。
 * - 個人情報や認証情報を扱う画面では、入力フィールドが適切にマスクされているかを確認するテストの視点を養います。
 *
 * @modification
 * - ログイン画面(login.jsp)のパスワード入力欄のinputタグのtype属性を "password" から "text" に変更しました。
 *
 * @impact
 * - ログイン画面(A101)でパスワードを入力すると、入力した文字がアスタリスク(*)で表示されず、平文で表示されるようになります。
 *
 * @revert
