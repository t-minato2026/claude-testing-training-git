...
public class TicketSearchServiceImpl implements TicketSearchService {
    ...
    public List<FlightVacantInfoDto> searchFlight(
         ...
         *
         * @purpose
         * - 仕様通りに作られていても、品質が高いとは限らないことを学習させます。特にエラーハンドリングにおけるユーザビリティが、ユーザー満足度に直結することを理解します。 
         *
         * @modification
         * - 空席照会時、運行していない区間が指定された場合に返却するエラーコードを、親切なメッセージに対応する`E_AR_B1_2002`から、不親切なメッセージに対応する`E_AR_B1_2003`に意図的に変更しました。
         *
         * @impact
         * - 空席照会画面(B101)で運行していない区間を検索した際に、原因が分かりにくいエラーメッセージが表示されるようになります。 
         *
         * @revert
