...
public class TicketReserveServiceImpl implements TicketReserveService {
    ...
    public int calculateTotalFare(List<Flight> flightList,
        ...
        for (Passenger passenger : passengerList) {
             ...
             *
             * @bug_id        14
             * @bug_title     計算ロジックの誤り(子供料金)
             * @author        Sota Toyama
             * @see           「FaRSeTトレーニング用のATRSのサンプル不具合の案」
             *
             * @purpose
             * - 複数の条件が絡み合う複雑なビジネスルールを、どのようにテストすればよいかを学習させます。
             * - 一つの機能が、他の情報(この場合は搭乗者情報)と正しく連携しているかを確認することの重要性を理解します。
             *
             * @modification
             * - 搭乗者の年齢をチェックし、子供の人数をカウントアップするif文をコメントアウトしました。
             *
             * @impact
             * - `calculateTotalFare`メソッド全体に影響します。
             * - この修正により、`childNum`が常に0で計算されるため、全員が大人料金で計算されるようになります。
             *
             * @revert
             * - このFaRSeTコメントブロックと直下のBUGGY CODEを削除し、その下にあるORIGINAL CODEのコメントアウト(/*と*&#47;)を解除してください。
             *
             ****************************************************************/
            // --- BUGGY CODE ---
            // 意図的にコメントアウト
     ...
     * @throws AtrsBusinessException チェック失敗例外
     */
    @SuppressWarnings("unused") // 不具合実装に伴う未使用変数の警告を抑制
    private void validateFareType(
        ...
        for (ReserveFlight reserveFlight : reserveFlightList) {
             ...
             *
             * @bug_id 1
             * @bug_title 割引条件のチェック漏れ
             * @author Sota Toyama
             * @see 「FaRSeTトレーニング用のATRSのサンプル不具合の案」
             *
             * @purpose
             * - 要件定義書(A04-02)で定義された割引条件のチェック処理を意図的に無効化する。
             * - ビジネスルールの考慮漏れという、機能の正確性に関する不具合を再現する。
             *
             * @modification
             * - レディース割、グループ割のチェックを行っている if-else if 文をコメントアウト。
             *
             * @impact
             * - チケット予約時の割引条件（レディース割、グループ割等）のチェック処理全体に影響。
             * - 通常運賃での予約や、他機能への影響はなし。
             *
             * @revert
